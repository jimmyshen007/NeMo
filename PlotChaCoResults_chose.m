function [] = PlotChaCoResults_chose(ChaCoResultsFileName,GBPlot,SurfPlot,BoxPlot,GraphPlot,figstr,plotlobecolor,KeepNPats)
...
elseif atsz==86
    AtlasName = ['/home/amy/work/BTFdata/Normals/e007573/T1scan_FS/Atlased' num2str(atsz) '/saT1_atlas86.nii'];
end
...
if atsz==116 && plotlobecolor
    ...
    C = zeros(116,116);
elseif atsz==86 && plotlobecolor
    load('/home/amy/Desktop/Matlab2009a/StructrualConnectivity_Bayes/Atlasing/Yeo_JNeurophysiol11_MNI152/Convert_86to7atlas.mat')
    lobes = Functional86_roi;
...
if GBPlot.flag ==1
    mL = [];
    for ii = 2:length(ChaCoResults);
        if ~isempty(strmatch(ChaCoResults(ii).NormPatID,KeepNPats));
            if size(ChaCoResults(ii).Regions,1)==1
                mL = [mL ChaCoResults(ii).Regions'];
            else
                mL = [mL ChaCoResults(ii).Regions];
            end
        end
    end
    brain_network_ploting(pth,at, C, abs(mean(mL')'), [], lobes, 'GBmeanChaCo',[],GBPlot.movie,strsave)
end
...
if SurfPlot.flag == 1
    ...
    pR = spl(find(spl>0))';
    nR = spl(find(spl<=0))';
    PlotIndex_p = floor((length(pR)-1)*(pR./max(pR))) + 1;
    PlotIndex_n = -(floor((length(nR)-1)*(nR./min(nR))) + 1);
    PlotIndex_n(isnan(PlotIndex_n)) = 1;
    PlotIndex(find(spl>0)) = PlotIndex_p;
    PlotIndex(find(spl<=0)) = PlotIndex_n;
    GummiBrain(pth,PlotIndex,AtlasName,SurfPlot.nMAP,SurfPlot.pMAP,SurfPlot.PlotHemi,ZonesName,lobes,'SPmeanChaCo',SurfPlot.movie,strsave)
end
...
if BoxPlot.flag == 1
    ...
    elseif length(unique(lobes))==8;
        ...
        map = repmat([1 0 0],[length(unique(lobes)) 1]);
    end
    bp = figure;
...
if GraphPlot.flag == 1
    load(['/mnt/extra/BTFdata/FiberTracts' num2str(atsz) '_MNI/AllConnMatrices' num2str(atsz) '_FT_MNI/nGraphMets' num2str(atsz) '_MNI']);
    netmet = [];
    nemtmet_NC = [];
    ...
    netmetm = ChaCoResults(1).nConMatMets;
    netmetom  = ChaCoResults(1).OrigMat.nMetsMapTFC;    
    bp = figure; 
    screenSize = get(0,'ScreenSize');
    gbp = get(bp,'Position');
    ...
    hold on
    plot(1,netmetom(1),'b.','MarkerSize',24)
    hold on
    ...
    hold on
    plot(1,netmetom(2),'b.','MarkerSize',24)
    hold on
    legend({'Mean Patient Matrix','Mean NC Matrix'},'Location','SouthWest','FontSize',12)
    hold on;
...
for i = 1:size(C,2)  
    ...
    if nargin >=4 && (length(unique(lobes))==6 || length(unique(lobes))==8)
...
function GummiBrain(pth,ColorIndex,AtlasName,nMAP,pMAP,PlotHemi,ZonesName,lobes,figstr,movie_flag,strsave)
...
%want to change this.
if nargin<3 || isempty(pMAP)
%     pMAP = colormap(jet(2*(length(find(ColorIndex>0))+1)));
%     pMAP = pMAP(1:length(find(ColorIndex>0))+1,:);
    pMAP = colormap(summer(length(find(ColorIndex>0))+1));
    pMAP = flipud(pMAP);
end
if nargin<3 || isempty(nMAP)
    %     nMAP = colormap(jet(2*(length(find(ColorIndex<=0))+1)));
    %     nMAP = nMAP(length(find(ColorIndex<=0))+1:end-1,:);
    if ~(size(pMAP,1)==1)
        nMAP = colormap(hot(length(find(ColorIndex<=0))+1));
        nMAP = flipud(nMAP);
        [~,mind] = min(sum((ones(size(nMAP,1),1)*pMAP(1,:)-nMAP).^2,2));
        nMAP = colormap(hot(length(find(ColorIndex<=0))+1+floor(100*mind/size(nMAP,1))));
        nMAP = flipud(nMAP);
        [~,mind] = min(sum((ones(size(nMAP,1),1)*pMAP(1,:)-nMAP).^2,2));
        nMAP = nMAP(mind:end,:);
        if ~(size(nMAP,1)==(length(find(ColorIndex<=0))+1));
            nMAP = [nMAP; ones(abs(size(nMAP,1)-(length(find(ColorIndex<=0))+1)),1)*nMAP(end,:)];
        end
    else
        nMAP = colormap(hot(length(find(ColorIndex<=0))+1));
        nMAP = flipud(nMAP);
         [~,mind] = min(sum((ones(size(nMAP,1),1)*[1 1 0.4]-nMAP).^2,2));
        nMAP = colormap(hot(length(find(ColorIndex<=0))+1+floor(100*mind/size(nMAP,1))));
        nMAP = flipud(nMAP);
        [~,mind] = min(sum((ones(size(nMAP,1),1)*[1 1 0.4]-nMAP).^2,2));
        nMAP = nMAP(mind:end,:);
...
for i = roinums';    
    if strcmp(PlotHemi,'left') && ~isempty(strfind(ZonesName(i,:),'_L')) && isempty(strfind(ZonesName(i,:),'_Lobule_R'))
        ...
        if ColorIndex(i)<=0
            myvol_render(plot3d,[1 1 1],nMAP,abs(ColorIndex(i)))
        else
            myvol_render(plot3d,[1 1 1],pMAP,ColorIndex(i))
        end
        hold on;
