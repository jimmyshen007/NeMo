function [] = PlotLoCoResults_chose(LoCoResultsFileName,GBPlot,SurfPlot,BoxPlot,GraphPlot,figstr, KeepNPats)
...
if GBPlot.flag ==1
    mL = [];
    for ii = 2:length(LoCoResults);
        if ~isempty(strmatch(LoCoResults(ii).NormPatID,KeepNPats));
            if size(LoCoResults(ii).Regions,1)==1
                mL = [mL LoCoResults(ii).Regions'];
            else
                mL = [mL LoCoResults(ii).Regions];
            end
        end
    end
    brain_network_ploting(pth,at, C, abs(mean(mL')'), [], lobes, 'GBmeanLoCo',[],GBPlot.movie,strsave)
end
...
if SurfPlot.flag == 1
    ...
    plr = mean(mL')';
    PlotIndex = floor((atsz-1)*(plr./max(plr))) + 1;
...
if GraphPlot.flag == 1
    if GraphPlot.Global == 1;
        ...
        for i = 2:length(LoCoResults);
            netmet(i-1,:) = LoCoResults(i).nConMatMets;
            netmet_NC(i-1,:) = LoCoResults(i).OrigMat.nMetsMapTFC;
        end
        netmetm = LoCoResults(1).nConMatMets;
        netmetom  = LoCoResults(1).OrigMat.nMetsMapTFC;
        bp = figure;
        screenSize = get(0,'ScreenSize');
        gbp = get(bp,'Position');
    ...
    if GraphPlot.Local == 1
        %figure;
        %imagesc(LoCoResults(1).nConMat)
        if isfield(GraphPlot.Local,'EF')
