function [] = PlotLoCoResults_ORIG(LoCoResultsFileName,GBPlot,SurfPlot,BoxPlot,GraphPlot,figstr)

[pth,fn,~] = fileparts(LoCoResultsFileName);
load(LoCoResultsFileName)
if strfind(fn,'_T1')
    strsave = ['T1_' figstr];
else
    strsave =figstr;
...
atsz = length(LoCoResults(1).Regions);
...
if GBPlot.flag ==1
    brain_network_ploting(pth,at, C, LoCoResults(1).Regions', [], lobes, 'GBmeanLoCo',[],GBPlot.movie,strsave)
end
%plot the gummi brain
if SurfPlot.flag == 1
    PlotIndex = floor((atsz-1)*(LoCoResults(1).Regions'./max(LoCoResults(1).Regions'))) + 1;
    GummiBrain(pth,PlotIndex,AtlasName,SurfPlot.MAP,SurfPlot.PlotHemi,ZonesName,lobes,'SPmeanLoCo',SurfPlot.movie,strsave)
end
%plot the boxplots of the LoCo scores across the normal subjects
if BoxPlot.flag == 1
    mL = [];
    if length(LoCoResults)==1
        mL = LoCoResults(1).AllRegions;
    else
        for i = 2:length(LoCoResults);
            mL = [mL LoCoResults(i).Regions'];
        end
    end
    ...
    boxplot(mL','plotstyle','compact','colors',map(lobes,:),'labels',ZonesName)
    I = getframe(bp);
    imwrite(I.cdata, [pth filesep 'LoCoPerRegion' strsave '.tif']);
end
if GraphPlot.flag == 1
    load(['/mnt/extra/BTFdata/FiberTracts' num2str(atsz) '_MNI/AllConnMatrices' num2str(atsz) '_FT_MNI/nGraphMets' num2str(atsz) '_MNI']);
    netmet = [];
    ecc = [];
    nOM = zeros(size(LoCoResults(2).nConMat));
    if length(LoCoResults)==1
        netmet = LoCoResults.AllnConMatMets;
        netmetm = LoCoResults.nConMatMets;
    else
        for i = 2:length(LoCoResults);
            D = distance_wei(1./LoCoResults(i).nConMat);
            [netmet(i-1,1),netmet(i-1,2),ecc(i-1,:),netmet(i-1,3),netmet(i-1,4)] = charpath(D);
            nOM = nOM + LoCoResults(i).OrigMat.nMapTFC;
        end
        nOM = nOM./(length(LoCoResults)-1);
        netmetm = [];
        Dm = distance_wei(1./LoCoResults(1).nConMat);
        [netmetm(1),netmetm(2),~,netmetm(3),netmetm(4)] = charpath(Dm);
        Dom = distance_wei(1./nOM);
        [netmetom(1),netmetom(2),~,netmetom(3),netmetom(4)] = charpath(Dom);
    end    
    bp = figure; 
    screenSize = get(0,'ScreenSize');
    gbp = get(bp,'Position');
