...
elseif atsz==86 && plotlobecolor
    load(['resource' filesep 'Convert_86to7atlas.mat']);
...
if SurfPlot.flag == 1
    ChaCoResults(1).Regions = ChaCoResults(1).Regions;
    pR = ChaCoResults(1).Regions(find(ChaCoResults(1).Regions>0))';
    nR = ChaCoResults(1).Regions(find(ChaCoResults(1).Regions<=0))';
    PlotIndex_p = floor((length(pR)-1)*(pR./max(pR))) + 1;
    PlotIndex_n = -(floor((length(nR)-1)*(nR./min(nR))) + 1);
    PlotIndex_n(isnan(PlotIndex_n)) = 1;
    PlotIndex(find(ChaCoResults(1).Regions>0)) = PlotIndex_p;
    PlotIndex(find(ChaCoResults(1).Regions<=0)) = PlotIndex_n;
...
if BoxPlot.flag == 1
    ...
    for i = 2:length(ChaCoResults);
        if size(ChaCoResults(i).Regions,1)==1
