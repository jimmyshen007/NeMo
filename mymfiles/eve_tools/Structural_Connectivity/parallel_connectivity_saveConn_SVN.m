function parallel_connectivity_saveConn_SVN(zones0,Zones,Path,qpath,atlassize)
...
banner=['parallel_connectivity_saveConn_generic ']';
bpt=length(banner);
...
parfor i = 1:size(zones,1)
    if mod(i,500)==0; disp(['Zone ' num2str(i) ' out of ' num2str(size(zones,1))]); end;
    Q = load([Path filesep qpath filesep 'qbelong' num2str(zones(i,1)) '_' num2str(zones(i,2))]);
...
plot(1:size(zones,2),1:size(zones,2),'x','Markersize',10,'color','w');
colorbar;
if ~isempty(strfind(qpath,'MNI'))
    save([Path filesep 'Matrix_ACS' num2str(atlassize) '_MNI'], 'MapACS');
    Connectivity_Map{1} = [Path filesep 'Matrix_ACS' num2str(atlassize) '_MNI'];
    save([Path filesep 'Matrix_ACD' num2str(atlassize) '_MNI'], 'MapACD');
    Connectivity_Map{2} = [Path filesep 'Matrix_ACD' num2str(atlassize) '_MNI'];    
    save([Path filesep 'Matrix_ACP' num2str(atlassize) '_MNI'], 'MapACP');
    Connectivity_Map{3} = [Path filesep 'Matrix_ACP' num2str(atlassize) '_MNI'];
else
    save([Path filesep 'Matrix_ACS' num2str(atlassize)], 'MapACS');
    Connectivity_Map{1} = [Path filesep 'Matrix_ACS' num2str(atlassize)];
    save([Path filesep 'Matrix_ACD' num2str(atlassize)], 'MapACD');
    Connectivity_Map{2} = [Path filesep 'Matrix_ACD' num2str(atlassize)];    
    save([Path filesep 'Matrix_ACP' num2str(atlassize)], 'MapACP');
    Connectivity_Map{3} = [Path filesep 'Matrix_ACP' num2str(atlassize)];
end
