function findfibre3MNI(Zones, zones0, NumbFiles,Path,PoscDir,qbelong_dir,Pbelong_dir)
...
cwd = cd;
if ~exist([Path filesep Pbelong_dir],'dir')
    mkdir([Path filesep Pbelong_dir])
end
if ~exist([Path filesep qbelong_dir],'dir')
    mkdir([Path filesep qbelong_dir])
end
%Path = uigetdir('','Dir to load files Poscs_ and Paths_, and to save results ...');
cd(Path)
if nargin < 3, NumbFiles = size(spm_get('files',Path,'PathsMNI_*.mat'),1); end
counter = 0;
...
for file = 1:NumbFiles
    ...
    if ~isempty(posc)
        for i = 1:size(posc,1)
            ...
            flag = 1;
            if Z1 > Z2, temp = Z2; Z2 = Z1; Z1 = temp; elseif Z1==Z2; flag = 0; end
            if ((~isempty(Z1)) && (~isempty(Z2))) && flag
...
for j = 1:size(zones,1)
    eval(['save ',Pbelong_dir,filesep,'Pbelong',num2str(zones(j,1)),'_',num2str(zones(j,2)),...
...
for i = 1:size(zones,1)
    ...
    eval(['load ' Pbelong_dir filesep 'Pbelong' num2str(zones(i,1)) '_' num2str(zones(i,2))]);
    ...
    for j = 1:size(Pbelong,1)
        if Pbelong(j,1) ~= fileAnt
            eval(['load PathsMNI_segsn' filesep 'Paths_' num2str(Pbelong(j,1))]);
            fileAnt = Pbelong(j,1);
        end
        eval(strcat('qbelong(',num2str(j),') = q(Pbelong(',num2str(j),',2));'));
    end
    eval(['save ' qbelong_dir filesep 'qbelong' num2str(zones(i,1)) '_' num2str(zones(i,2))...
        ' qbelong']);
