function amy_Tracking_SVN(tracking,seed,seed_zone,Tzones,Zones,Diff_name,inf_name,Mask_name,Atlas_name,step,longMax,NumXfiles,repetitions,ang,numVectors,OutFilePath,Ppaths,seed_begin)
...
[mpth,mfn,mext] = fileparts(Mask_name);
if strcmp(mext,'.img')
    MaskVol.pinfo = [1 0 0]';
end
...
[apth,afn,aext] = fileparts(Atlas_name);
if strcmp(aext,'.img')
...
banner=['amy_Tracking_SVN ']';
bpt=length(banner);

savestr = (floor(seed_begin/NumXfiles)+1):1:(floor(num_seed/NumXfiles)+1);
k = 0;

for j = seed_begin:num_seed
    if mod(j,1000)==0; disp(['Seed ' num2str(j) ' out of ' num2str(num_seed) ' ' banner(1+mod(j/1000,bpt))]); end
     seed_r = seed(j,:);
