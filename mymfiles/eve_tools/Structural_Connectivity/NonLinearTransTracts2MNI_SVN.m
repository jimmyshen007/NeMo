function [mbeg,mend,numf,time_to_convert] = NonLinearTransTracts2MNI_SVN(qbelongoriginal_dir,sT1scan,sT1scan_Atlas,wT1scan,atlassize,MRIOPT)
    ...
    if ~(size(q,2)==1)
        for j = 1:size(q,2)
            if ~isempty(q(j).voxels)
                ...
                [txyz,tval] = nonlin_transform_coord(mask,Def);
                qnew(j).PT = q(j).PT;
                qnew(j).voxels = txyz;
                qnew(j).bme = tval;
                ...
                tind =sub2ind(size(Def.X),txyz(:,1),txyz(:,2),txyz(:,3));
