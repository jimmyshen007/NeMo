function [newDiffFileNames, newGMMaskFileName, newWMMaskFileName, newContMaskFileName, newAtlasFileName] = resampling2DWMRI_SVN2(DiffFileNames, GMMaskFileName, WMMaskFileName, ContMaskFileName, AtlasFileName,T1FileName,MRIOPT,ResliceDTIimages)
...
if min(size(newDiffFileNames))==1
    b0Image = newDiffFileNames;
else
...
%Coreg start...
flags = spm_get_defaults;
flags.coreg.write.prefix = 's';
flags.coreg.write.interp = 0;
x = spm_coreg(b0Image,newT1FileName,flags);
M = inv(spm_matrix(x));
M_DTI = spm_get_space(b0Image);
M_T1 = spm_get_space(T1FileName);
save([pp filesep 'LinearTrans_T12DTI'],'M','M_T1','M_DTI')
spm_get_space(newT1FileName,M*spm_get_space(newT1FileName))
spm_get_space(newAtlasFileName,M*spm_get_space(newAtlasFileName))
spm_get_space(newContMaskFileName,M*spm_get_space(newContMaskFileName))
spm_get_space(newWMMaskFileName,M*spm_get_space(newWMMaskFileName))
spm_get_space(newGMMaskFileName,M*spm_get_space(newGMMaskFileName))

rflags.mask = 0; rflags.mean = 0; rflags.interp = 0; rflags.which = 1; rflags.prefix = 's';
spm_reslice({b0Image,newGMMaskFileName,newWMMaskFileName,newContMaskFileName,newAtlasFileName,newT1FileName},rflags)
%... coreg end
