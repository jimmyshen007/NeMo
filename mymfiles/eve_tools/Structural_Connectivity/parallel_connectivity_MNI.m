function parallel_connectivity_MNI(zones0,Zones,Path)
...
for i = 1:size(zones,1)
    ...
    Q = load([Path filesep 'qbelongMNI_origPoscs' filesep 'qbelong' num2str(zones(i,1)) '_' num2str(zones(i,2))]);
    ROI1 = Zones(zones(i,1)).roi;
    ROI2 = Zones(zones(i,2)).roi;
    [ACS, ACD, ACP] = Areas_MNI(ROI1,ROI2,Q.qbelong,zones(i,1),zones(i,2),Path);
    Q = [];
...
colorbar; Map = MapACS; save([Path filesep 'Matrix_ACS_MNI_origPoscs'], 'Map');
Connectivity_Map{1} = [Path filesep 'Matrix_ACS_MNI_seg_sn'];
...
plot(1:size(zones,2),1:size(zones,2),'x','Markersize',10,'color','w');
colorbar; Map = MapACD;save([Path filesep 'Matrix_ACD_MNI_origPoscs'], 'Map');
Connectivity_Map{2} = [Path filesep 'Matrix_ACD_MNI_seg_sn'];
...
plot(1:size(zones,2),1:size(zones,2),'x','Markersize',10,'color','w');
colorbar; Map = MapACP; save([Path filesep 'Matrix_ACP_MNI_origPoscs'], 'Map');
Connectivity_Map{3} = [Path filesep 'Matrix_ACP_MNI_seg_sn'];